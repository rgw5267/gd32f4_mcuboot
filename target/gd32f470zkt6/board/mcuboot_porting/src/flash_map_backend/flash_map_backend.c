...
#define APPLICATION_PRIMARY_START_ADDRESS   0 // phy start addr is 64 * 1024
...
#define FLASH_SECTOR_SIZE                   MCU_FLASH_PAGE_SIZE
...
{
    for (size_t i = 0; i < m_ARRAY_SIZE(s_flash_areas); i++)
    {
                    ...
                    void *dst, uint32_t len)
{
    ...
    {
        fal_part = fal_partition_find("boot");
    }
    ...
    {
        fal_part = fal_partition_find("secondary");
    }
    if(fal_part == NULL)
    {
        MCUBOOT_LOG_ERR("%s: fal partition not found", __func__);
        return -1;
    }
    fal_partition_read(fal_part, off, dst, len);
                     ...
                     const void *src, uint32_t len)
{
    ...
    fal_partition_write(fal_part, off, src, len);
                     ...
                     uint32_t off, uint32_t len)
{
    ...
    fal_partition_erase(fal_part, off, len);
